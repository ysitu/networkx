...
class BaseDiGraphTester(BaseGraphTester):
    ...
    def test_edges(self):
        G = self.K3
        assert_equal(
            sorted(G.edges()), [(0, 1), (0, 2), (1, 0), (1, 2), (2, 0), (2, 1)])
    ...
    def test_in_degree_iter_weighted(self):
        ...
        assert_equal(
            list(G.in_degree_iter(weight='weight')), [(0, 2), (1, 1.3), (2, 2)])
        assert_equal(
            dict(G.in_degree_iter(weight='weight')), {0: 2, 1: 1.3, 2: 2})
        assert_equal(list(G.in_degree_iter(1, weight='weight')), [(1, 1.3)])
        assert_equal(
            list(G.in_degree_iter(weight='other')), [(0, 2), (1, 2.2), (2, 2)])
        assert_equal(
            dict(G.in_degree_iter(weight='other')), {0: 2, 1: 2.2, 2: 2})
    ...
    def test_out_degree_iter_weighted(self):
        ...
        G.add_edge(0, 1, weight=0.3, other=1.2)
        assert_equal(
            list(G.out_degree_iter(weight='weight')), [(0, 1.3), (1, 2), (2, 2)])
        assert_equal(
            dict(G.out_degree_iter(weight='weight')), {0: 1.3, 1: 2, 2: 2})
        assert_equal(list(G.out_degree_iter(0, weight='weight')), [(0, 1.3)])
        assert_equal(
            list(G.out_degree_iter(weight='other')), [(0, 2.2), (1, 2), (2, 2)])
        assert_equal(
            dict(G.out_degree_iter(weight='other')), {0: 2.2, 1: 2, 2: 2})
...
class TestDiGraph(BaseAttrDiGraphTester, TestGraph):
    ...
    def test_add_edges_from(self):
        ...
        # too many in tuple
        assert_raises(networkx.NetworkXError, G.add_edges_from, [(0, 1, 2, 3)])
